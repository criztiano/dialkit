  ...
  return (
    <div className="tweakers-root lib-page" data-theme={theme}>
      ...
      <main className="lib-main">
        ...
        <Section index="14" title="Analyser" hint="A real-time trace of streaming audio, read from a Web Audio AnalyserNode every frame. Press Play to start two live channels (a drone and a blip loop), then flip frequency / waveform / ekg, area / line, smooth / pixelated, log / linear, and the spring. The top-right buttons mute or solo a channel — the demo wires them to the channel gains. The last two sources compare two signals: transfer plots input against output as a curve, overlay lays both waveforms on one axis — the demo feeds them the mix and a saturated copy of it." single>
          <Card title="AnalyserVisualization" desc="Displays a live spectrum (frequency), oscilloscope (waveform), medical-monitor trace (ekg — a pen dot fixed at the right edge rides the signal's level while its history streams left), transfer curve, or two-signal overlay from an AnalyserNode it only ever reads — fftSize, smoothingTimeConstant, and the dB window stay yours. Area fills the trace translucently with a crisp outline — under the spectrum, around the waveform's min/max band; line is the trace alone. Smooth is an interpolated curve; pixelated is chunky blocks in the waveform visualizer's pixel language (pixelSize 1/2/4/6). The spring option smooths movement render-side and can overshoot — it composes with the analyser's own data-side smoothing. muted/soloed are controlled props with onMuteChange/onSoloChange callbacks (buttons appear only when wired); the host owns the actual gain routing. waveColor and fillColor are themable; with no analyser it rests at a baseline. The transfer and overlay sources take a second tap (analyserB) — the processed output the input is measured against: transfer is an XY plot where every sample lands at (input, output), so a memoryless shaper draws its curve and time-dependent processing opens it into loops (draw it as connected segments or as scatter dots for noisy signals); overlay lays both waveforms on one axis, input behind and output in front, locked to a rising zero crossing so periodic tones hold still, with windowSize as the time zoom." code="<AnalyserVisualization analyser analyserB source variant mode spring transferDraw windowSize muted onMuteChange soloed onSoloChange />">
            <AnalyserShowcase />
          </Card>
        </Section>
