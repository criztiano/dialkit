...
export function createTweakTimeline<T extends TimelineConfig>(
...
): Accessor<TweakTimelineValues<T>> {
  ...
  const [flatValues, setFlatValues] = createSignal(TweakStore.getValues(panelId));
  ...
  onMount(() => {
    TweakStore.registerPanel(panelId, name, parsed().tweakConfig, undefined, {
