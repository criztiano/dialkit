...
import { DialStore, parseListItemSchema, hintDomId, type ControlMeta } from '../src/store/DialStore';
...
const register = (
  ...
  DialStore.registerPanel(id, id, config, undefined, { hints });
  registered.push(id);
};
...
describe('control hints', () => {
  ...
  it('replaces the whole map when an updatePanel does restate it', () => {
    ...
    DialStore.updatePanel(id, id, config, undefined, { hints: { 'physics.frozen': 'Pause the sim.' } });
...
describe('hint tooltip ids', () => {
  ...
  it('collapses whitespace so the id stays a single reference', () => {
    expect(hintDomId('Photo Stack-:r1:', 'shadowTint')).toBe('dialkit-hint-Photo-Stack-:r1:-shadowTint');
    expect(hintDomId('Photo Stack', 'x')).not.toContain(' ');
  });

  it('leaves a whitespace-free id alone', () => {
    expect(hintDomId('panel-1', 'physics.elastic')).toBe('dialkit-hint-panel-1-physics.elastic');
  });
});
