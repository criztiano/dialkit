...
describe('multiselect config', () => {
  it('resolves to its default selection', () => {
    ...
    expect(DialStore.getValues(id).models).toEqual(['a', 'c']);
  });

  it('defaults to the empty selection — a real state, not the first option', () => {
    const id = freshId();
    register(id, models());
    expect(DialStore.getValues(id).models).toEqual([]);
  });
  ...
  it('stores an updated selection', () => {
    const id = freshId();
    register(id, models());
    DialStore.updateValue(id, 'models', ['b', 'c']);
    expect(DialStore.getValues(id).models).toEqual(['b', 'c']);
  });
});
...
describe('multiselect reconciliation across config edits', () => {
  it('keeps a valid selection on a config update', () => {
    const id = freshId();
    register(id, models());
    DialStore.updateValue(id, 'models', ['a', 'b']);
    DialStore.updatePanel(id, id, models());
    expect(DialStore.getValues(id).models).toEqual(['a', 'b']);
  });

  it('drops selections whose option vanished', () => {
    const id = freshId();
    register(id, models());
    DialStore.updateValue(id, 'models', ['a', 'b', 'c']);
    const narrowed = {
      models: {
        type: 'multiselect' as const,
        options: [
    ...
    DialStore.updatePanel(id, id, narrowed);
    expect(DialStore.getValues(id).models).toEqual(['a', 'c']);
  });
  ...
  it('keeps an explicitly emptied selection instead of restoring the default', () => {
    ...
    DialStore.updateValue(id, 'models', []);
    DialStore.updatePanel(id, id, models(['a']));
    expect(DialStore.getValues(id).models).toEqual([]);
  });
  ...
  it('falls back to the default when the stored value is not a string array', () => {
    const id = freshId();
    register(id, models(['a']));
    DialStore.updateValue(id, 'models', 'not-an-array');
    DialStore.updatePanel(id, id, models(['a']));
    expect(DialStore.getValues(id).models).toEqual(['a']);
  });
});
...
describe('slider object config', () => {
  it('resolves to its default and carries unit through control meta', () => {
    ...
    expect(DialStore.getValues(id).threshold).toBe(-24);
    const control = DialStore.getPanel(id)!.controls[0];
  ...
  it('infers the step when omitted', () => {
    ...
    expect(DialStore.getPanel(id)!.controls[0].step).toBe(1);
  });

  it('is not mistaken for a folder', () => {
    const id = freshId();
    register(id, { gain: { type: 'slider' as const, default: 1, min: 0, max: 2 } });
    expect(DialStore.getPanel(id)!.controls[0].type).toBe('slider');
    expect(DialStore.getPanel(id)!.controls[0].children).toBeUndefined();
  });
});
