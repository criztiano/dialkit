...
describe('range value reconciliation across config edits', () => {
  ...
  it('keeps a valid in-bounds pair on a config update', () => {
    ...
    TweakStore.updateValue(id, 'size', { min: 20, max: 80 });
    TweakStore.updatePanel(id, id, rangeConfig(0, 100));
    expect(TweakStore.getValues(id).size).toEqual({ min: 20, max: 80 });
  });

  it('clamps an out-of-bounds pair to the new bounds', () => {
    const id = freshId();
    register(id, rangeConfig(0, 100));
    TweakStore.updateValue(id, 'size', { min: -50, max: 200 });
    // Tighten the bounds; the stored pair must clamp into [10, 90].
    TweakStore.updatePanel(id, id, rangeConfig(10, 90));
    expect(TweakStore.getValues(id).size).toEqual({ min: 10, max: 90 });
  });
  ...
  it('re-orders a pair that reverses after clamping', () => {
    ...
    TweakStore.updateValue(id, 'size', { min: 45, max: 20 });
    TweakStore.updatePanel(id, id, rangeConfig(30, 40));
    const value = TweakStore.getValues(id).size as { min: number; max: number };
  ...
  it('falls back to the config default when the stored value is not {min,max}', () => {
    ...
    TweakStore.updateValue(id, 'size', 'not-a-range');
    TweakStore.updatePanel(id, id, { size: { type: 'range' as const, min: 0, max: 100, default: { min: 25, max: 75 } } });
    expect(TweakStore.getValues(id).size).toEqual({ min: 25, max: 75 });
  });
});
...
describe('range reconciliation with missing bounds', () => {
  it('does not produce NaN when a bound is absent', () => {
    ...
    const store = TweakStore as unknown as {
