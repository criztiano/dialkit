...
describe('captioned actions', () => {
  it('keeps the caption and the button text apart', () => {
    ...
    const action = DialStore.getPanel(id)!.controls[0];
  ...
  it('leaves a plain action uncaptioned', () => {
    ...
    expect(DialStore.getPanel(id)!.controls[0].caption).toBeUndefined();
  });
  ...
  it('still falls back to the key for the button text', () => {
    ...
    expect(DialStore.getPanel(id)!.controls[0].label).toBe('Load Sample');
  });
  ...
  it('is presentation, never a value', () => {
    ...
    expect(DialStore.getValue(id, 'load.caption')).toBeUndefined();
