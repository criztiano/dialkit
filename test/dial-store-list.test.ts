...
import { DialStore, normalizeListItems, type ListConfig, type ListItemValue } from '../src/store/DialStore';
...
describe('list row titles', () => {
  ...
  it('preserves a renamed row across a panel config update', () => {
    ...
    DialStore.updateValue(id, 'bodies', [{ type: 'body', params: { mass: 1 }, title: 'chair.glb' }]);
    DialStore.updatePanel(id, id, { bodies: listConfig([{ type: 'body', params: { mass: 1 } }]) });
    expect(DialStore.getValues(id).bodies).toEqual([
