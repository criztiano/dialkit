...
function isTransitionConfig(value) {
  return (0, import_TweakStore.isSpringConfigValue)(value) || (0, import_TweakStore.isEasingConfigValue)(value);
}
...
function parseTimelineConfig(config) {
  ...
  entries.forEach(({ path, childKey, group, clip }, index) => {
    ...
    clips.push({
      key: path,
      label: (0, import_TweakStore2.formatLabel)(childKey),
...
function inferFromToRange(key, value, counterpart) {
  ...
  return [value, min, max, (0, import_TweakStore2.inferStep)(min, max)];
}
...
function computeStaticClips(parsed, flatValues) {
  const resolved = (0, import_TweakStore2.resolveTweakValues)(parsed.tweakConfig, flatValues);
...
function parseHex(hex) {
  if (!(0, import_TweakStore2.isHexColor)(hex)) return null;
  let h = hex.slice(1);
...
function formatStepLabel(stepKey) {
  ...
  return match ? `Step ${match[1]}` : (0, import_TweakStore2.formatLabel)(stepKey);
}
...
function buildCopyInstruction(hookName, panelName, values) {
  const { [import_TweakStore3.TAB_PATH]: _activeTab, ...parameters } = values;
