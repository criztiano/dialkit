# Handoff — tabbed side-panel variant (dialkit)
...
| | Path | Role |
|---|---|---|
| Library | `/Users/cripto/Code/dialkit` | Where this work happens. Branch `main`. |
| Consumer | `/Users/cripto/Code/tracker/.claude/worktrees/sample-loading-ui-b08657` | Audioground tracker, branch `claude/poc-ui-dialkit-redesign-3f7160`. Real-world proof. |

The tracker consumes dialkit through `"dialkit": "file:/Users/cripto/Code/dialkit"`.

**Two traps, both cost time if you miss them:**

1. **npm `dialkit@1.4.3` is a DIVERGENT line, not an ancestor.** Local `main`
   ...
   until dialkit is published from current `main`. That is a known, accepted,
...
2. **pnpm COPIES `file:` deps into its store; it does not live-link them.**
   After any dialkit change the tracker needs all three of:
   ```bash
   cd /Users/cripto/Code/dialkit && npm run build
   ...
   `--ignore-scripts` avoids re-running dialkit's `prepare` build inside the
   consumer install.
...
| File | Why |
|---|---|
| `src/components/DialRoot.tsx` | Renders panels; `mode: 'popover' \| 'inline'` lives here (`DialRootProps`, ~line 15). Tabs are probably a third axis, not a third mode. |
| `src/components/Folder.tsx` | The root panel is a `Folder` with `isRoot`. Has `toolbar`, `collapsible`, `defaultOpen`. Root title row is `.dialkit-folder-title-root`. |
| `src/components/ControlRenderer.tsx` | The existing headerTabs hoist (150-176). |
| `src/store/DialStore.ts` | 2000+ lines. `registerPanel` / `updatePanel` / `normalizePreservedValue` (~738-744) — **existing store values win over new config defaults**, which is why tab state must be a real value if it should persist. |
| `src/styles/theme.css` | ~4600 lines, all tokens on the `.dialkit-root` CLASS (not `:root`). Panel styles around `.dialkit-panel-inner`, `.dialkit-folder-*`, `.dialkit-module-*`. |
...
From `skills/dialkit-integration/SKILL.md` (the doctrine doc; read it):
...
```bash
cd /Users/cripto/Code/dialkit && npm run build && npm test && npx tsc --noEmit
```
...
```bash
cd /Users/cripto/Code/dialkit/example && npm run dev
```
...
`src/dialkit-panel/panelConfig.ts` — top-level folders today are `pattern`,
...
- dialkit is unpublished relative to this work → tracker CI is red. Cri's call.
